...
const FilterLocationMenu = () => {
  const { state: { filter, activeFilterSection }, dispatch} = useContext(ProductFilterContext);
  const [location, setLocation] = useState(filter.location);

  const handleClick = useCallback(() => {
    dispatch({ type: ProductFilterActionTypes.SET_SECTION, payload: ProductFilterSections.LOCATION});
  }, [dispatch]);
  ...
  const handleApply = useCallback(() => {
    dispatch({ type: ProductFilterActionTypes.SET_FILTER_LOCATION, payload: location})
    dispatch({ type: ProductFilterActionTypes.SET_SECTION, payload: ProductFilterSections.NONE});
  }, [dispatch, location]);

  return (
    <div className={"filter-category-menu relative group " + (activeFilterSection === ProductFilterSections.LOCATION ? 'active' : '')}>
      ...
      <div className='filter-menu__panel hidden group-[.active]:block absolute transition bg-main-light px-6 pt-2 z-50 w-screen max-w-[370px] rounded-lg'>
        <div className='text-main-weighted text-lg font-semibold mt-2 mb-3'>Item condition</div>
        <div className='mb-3'>
          <input className='rounded-full w-full py-2.5 px-5 border border-main-thick' placeholder='Find your city'
            value={location}
            onChange={e => setLocation(e.target.value)}
          />
        </div>
        <div className='mb-3'>
          <div className='h-[222px] bg-main rounded-md'>
