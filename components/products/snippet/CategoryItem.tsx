...
const CategoryItem: React.FC<CategoryItemProps> = ({category}) => {
  const { state: { filter }, dispatch} = useContext(ProductFilterContext);

  const handleClick = useCallback(() => {
    if (filter.category === category.key) return;
    dispatch({ type: ProductFilterActionTypes.SET_FILTER_CATEGORY, payload: category.key});
  ...
  return (
    <div className={'w-1/5 h-[90px] flex items-center justify-center flex-col gap-y-2 hover:bg-main hover:opacity-80 transition cursor-pointer '
