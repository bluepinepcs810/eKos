...
import { ProductFilterContext, ProductFilterSections, ProductPriceFilterType } from './context/filter-context';
import { ProductFilterActionTypes } from './context/fitler-reducer';

const FilterPriceMenu = () => {
  const { state: { activeFilterSection, filter }, dispatch} = useContext(ProductFilterContext);
  const [range, setRange] = useState<ProductPriceFilterType>(filter.price);

  const handleClick = useCallback(() => {
    dispatch({ type: ProductFilterActionTypes.SET_SECTION, payload: ProductFilterSections.PRICE});
  }, [dispatch]);
  ...
  const handleApply = useCallback(() => {
    dispatch({ type: ProductFilterActionTypes.SET_FILTER_PRICE_RANGE, payload: range})
    dispatch({ type: ProductFilterActionTypes.SET_SECTION, payload: ProductFilterSections.NONE});
  }, [dispatch, range])

  return (
    <div className={"filter-price-menu relative group " + (activeFilterSection === ProductFilterSections.PRICE ? 'active' : '' )}>
      ...
      <div className='filter-menu__panel hidden group-[.active]:block absolute transition bg-main-light pt-2 px-6 z-50 w-screen max-w-[370px] rounded-lg'>
        <div className='text-main-weighted text-lg font-semibold mt-2 mb-3'>Price range</div>
        <div className='flex justify-between gap-x-3.5'>
          <div className='flex-grow'>
            ...
            <div>
              <input id="price_range_from" className='w-full border border-[#E3C0FF] py-2 px-2.5'
                placeholder='0'
                type="number"
                value={range.from}
                onChange={(e) => setRange(old => ({...old, from: parseInt(e.target.value)}))}
              />
            </div>
          </div>
          <div className='flex-grow'>
            ...
            <div>
              <input id="price_range_to" className='w-full border border-[#E3C0FF] py-2 px-2.5'
                placeholder='No limit'
                type="number"
                value={range.to}
                onChange={(e) => setRange(old => ({...old, to: parseInt(e.target.value)}))}
              />
            </div>
          </div>
        </div>
        <div className='mt-4 mb-4 flex justify-end gap-x-2'>
          ...
          <button className="rounded-full text-main-light border border-main-dark px-5 py-2 filled-button"
            onClick={handleApply}
          >
            Apply
          </button>
