...
const FilterCategoryMenu = () => {
  ...
  const category = useMemo(() => findCategoryItem(filter.category), [filter.category]);

  const handleClick = useCallback(() => {
    dispatch({ type: ProductFilterActionTypes.SET_SECTION, payload: ProductFilterSections.CATEGORY});
  }, [dispatch])
  ...
  return (
    <div className={"filter-category-menu relative group " + (activeFilterSection === ProductFilterSections.CATEGORY ? 'active' : '')}>
      ...
      <div className='filter-menu__panel hidden group-[.active]:block absolute transition bg-main-light pt-2 px-6 z-50 w-screen max-w-[580px] rounded-lg'>
        <div className='text-main-weighted text-lg font-semibold mt-2 mb-3'>Category</div>
