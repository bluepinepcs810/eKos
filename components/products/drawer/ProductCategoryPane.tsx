...
type ProductCategoryPaneProps = {
  ...
  setCategory: (type: CategoryItemType) => void;
};

const ProductCategoryPane: FC<ProductCategoryPaneProps> = ({
  setPane,
  setCategory,
}) => {
  return (
    <div className="bg-main-light h-full overflow-y-auto pt-12 px-3 md:pl-32 md:pr-2">
      <div className="flex flex-col">
        ...
        {CATEGORIES.map((category) => (
          <button
            className="flex items-center border-b border-third-main py-4 gap-x-3"
            key={category.key}
