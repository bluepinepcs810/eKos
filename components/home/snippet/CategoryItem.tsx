...
type CategoryItemReactProps = HTMLAttributes<HTMLDivElement> &
  CategoryItemProps;
const CategoryItem: React.FC<CategoryItemReactProps> = ({
  category,
  className,
  ...rest
}) => {
  ...
  return (
    <div
      className={
        `rounded-md w-[130px] h-[130px] flex flex-col justify-center items-center gap-3 text-main-weighted transition ${className} ` +
    ...
    >
      ...
      <div
        className={
          'text-center px-2 text-sm ' + (hover ? 'text-main-light' : '')
        }
      >
        {category.text}
      </div>
