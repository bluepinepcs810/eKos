...
const BuyingPane = () => {
  const { data, isError, error } = useSellingOrders(1);

  const handleStatusChange = (orderId: ID, value: OrderStatusEnum) => {
    // TODO add status change api
  };
              ...
              <td className="whitespace-nowrap text-sm text-main-weighted py-2 px-3">
                <div className="flex gap-x-3">
                  ...
                  <div className="flex items-center">
                    <div>{truncateString(order.buyer.userName, 10)}</div>
                  </div>
                </div>
              </td>
              <td className="whitespace-nowrap text-sm text-main-weighted py-2 px-3">
