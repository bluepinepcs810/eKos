...
const Thumb = ({
  ...
  return (
    <div className={'r-drop-thumb-item relative'}>
      <div
        className="absolute right-1 top-1 rounded-full  w-6 h-6 flex justify-center items-center bg-main-weighted text-white hover:bg-main-dark cursor-pointer"
        onClick={onRemove}
      >
        <div>X</div>
      </div>
...
const RDropzone = ({
  ...
  const onDrop = useCallback(
    (acceptedFiles: any) => {
      let serializableFiles: FileType[] = acceptedFiles.map(
        (file: File): FileType => {
      ...
      removeElement(serializableFiles, ({ file: file1, url: url1 }) => {
        return files.find(({ file: file2, url: url2 }) => {
          if (!file1) {
            return url1 === url2;
          }
  ...
  const cancelFile = useCallback(
    (file: FileType) => {
      const targetId = files.findIndex((item) => {
        if (!file.file) {
          return file.url === item.url;
        }
        if (!item.file) return false;
        return (
          file.file.type === item.file.type &&
    ...
    <>
      ...
      <div className="flex gap-x-4">
        {files.map((file: FileType, idx: number) => (
          <Thumb file={file} idx={idx} onClose={cancelFile} key={idx} />
        ))}
      </div>
    </>
  );
